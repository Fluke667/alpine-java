...
ENV JAVA_VERSION_MAJOR=8 \
    ...
    GLIBC_LOCALE="GLIBC_LOCALE=${GLIBC_LOCALE_INPUT}.${GLIBC_LOCALE_CHARMAP}" \
    GLIBC_LOCALE_INPUT=en_US \
    GLIBC_LOCALE_CHARMAP=UTF-8
...
RUN apk add --update --no-cache libstdc++ curl ca-certificates bash java-cacerts wget && \
    ...
    rm -v /tmp/*.apk && \
    /usr/glibc-compat/bin/localedef --inputfile ${GLIBC_LOCALE_INPUT} --charmap ${GLIBC_LOCALE_CHARMAP} ${GLIBC_LOCALE} && \
